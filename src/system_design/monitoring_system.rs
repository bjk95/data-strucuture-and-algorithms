use std::{collections::HashMap, cmp::{max, Ordering}};
...
impl Application {
    fn new() -> Self {
        Application { latency: HashMap::new(), errors: HashMap::new() }
    }
...
impl MonitoringSystem {
    ...
    fn log_latency(&mut self, application_name: String, api: String, latency_in_mills: i32) {
        ...
        self.applications.insert(application_name, updated_application);
    }
    ...
    fn get_percentile_latency(&self, percentile: i32, application_name: String, api: String) -> i32 {
    ...
    fn get_top_errors(&self, application_name: String, api: String) -> Vec<i32> {
        self.applications.get(&application_name).unwrap().get_top_error_codes(api)
    }
}
...
mod monitoring_system_tests {
    ...
    fn insert_latency_to_new_app(){
        ...
        assert_eq!(ms.applications.get(&app_name).unwrap().latency.get(&api_name).unwrap(), &vec![latency1, latency2, latency3, latency4]);
    ...
    fn error_test(){
        ...
        assert_eq!(ms.get_top_errors(app_name, api_name), vec![error1, error4, error3]);
    }
}
